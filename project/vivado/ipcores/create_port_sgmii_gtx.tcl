...
if {$part_family == "7series"} {
    ...
    if ![info exists bufg_freq_hz] {
        variable bufg_freq_hz 200000000
    }
    set shared_qpll true
    ...
    # Some Ultrascale/Ultrascale+ parts need DRP clock = 50MHz, but can override
    if ![info exists bufg_freq_hz] {
        variable bufg_freq_hz 50000000
    }
    variable bufg_freq_mhz [expr {round($bufg_freq_hz / 1000000)}]
    set shared_qpll false
    generate_sgmii_gtx $sgmii_gtx_loc sgmii_gtx0 1 $sgmii_gtx_refclk_mhz $bufg_freq_mhz
    generate_sgmii_gtx $sgmii_gtx_loc sgmii_gtx1 0 $sgmii_gtx_refclk_mhz $bufg_freq_mhz
...
# Set parameters
ipcore_add_param AUTONEG_EN bool $AN_EN\
...
ipcore_add_param SHARED_QPLL bool $shared_qpll\
