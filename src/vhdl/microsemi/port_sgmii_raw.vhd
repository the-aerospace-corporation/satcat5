...
--
-- This module supports up to a quad of transceivers (four lanes maximum). If
-- multiple quads share a single reference clock, it will need to be generated
-- by one quad and distributed to the rest using SHARED_EN:
...
--  * Link each "shared_in" port to "shared_out" port on the first instance. The
-- Enhanced Receiver Management reset must be shared across a quad, so a single
-- lane reset only has partial coverage. Full reset is done by setting reset_p
-- to all ones.
--
...
port (
    -- Clocking and reset
    clk_40      : in  std_logic;        -- 40MHz For ERM, required.
    gtrefclk_p  : in  std_logic := '0'; -- 125MHz reference, may be unused.
    gtrefclk_n  : in  std_logic := '0';
    reset_p     : in  std_logic_vector(LANES-1 downto 0); -- See note above.
    ...
    -- Shared-logic (clock forwarding to other quads) enabled?
...
signal reset_n          : std_logic_vector(LANES-1 downto 0);
signal reset_quad_n     : std_logic;
...
signal tx_reset_async   : std_logic_vector(LANES-1 downto 0);
...
-- Clocking and reset
reset_n <= not reset_p;
reset_quad_n <= not and_reduce(reset_p);

-- Shared logic distribution: reference clock provided via shared signals.
gen_shared0 : if not SHARED_EN generate
    xcvr_ref_clk <= shared_in(0);
end generate;

-- Shared logic distribution: generate the transceiver refclk for distribution
-- to this core and any downstream TX_PLL_SGMII instances.
gen_shared1 : if SHARED_EN generate
    u_xcvr_ref_clk : PF_XCVR_REF_CLK_SGMII
        port map(
        REF_CLK_PAD_P       => gtrefclk_p,
        REF_CLK_PAD_N       => gtrefclk_n,
        REF_CLK             => xcvr_ref_clk);
end generate;

-- Forward the transceiver reference clock (may be unused).
shared_out <= (0 => xcvr_ref_clk, others => '0');

-- All lanes in this core share a PLL, can use either TXPLL or TXPLL_SSC.
...
-- RX/TX/full lane resets
rx_reset_async <= reset_p or not rx_locked;
tx_reset_async <= reset_p or not tx_locked;
lane_reset_async <= rx_reset_async or tx_reset_async;
...
gen_lanes : for n in 0 to LANES-1 generate
    ...
    port map(
        ...
        LANE0_RX_IDLE           => open,
        LANE0_RX_READY          => open,
        LANE0_RX_VAL            => rx_locked(n), -- Ready and ERM OK.
