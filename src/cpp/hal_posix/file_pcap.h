...
//!  * [PCAP v2.4](https://datatracker.ietf.org/doc/id/draft-gharris-opsawg-pcap-00.html)
//!  * [PCAPNG](https://www.ietf.org/archive/id/draft-tuexen-opsawg-pcapng-02.txt)
//!
...
namespace satcat5 {
    namespace io {
        ...
        public:
            ...
            //! An interface description block is required for each capture
            //! interface. The default (ID = 0) is created by `open`.
            //! \returns The interface-ID for the newly-created interface.
            ...
            inline void set_interface(u32 id)
                { m_interface_next = id; }
        ...
        protected:
            ...
            u32 m_interface_count, m_interface_next;
            u8 m_buff[SATCAT5_PCAP_BUFFSIZE];
        };

        //! Helper object for writing capture files with multiple sources.
        //! PCAP-NG files can interleave packets from different network
        //! interfaces. This class represents one such interface, passing
        //! metadata to the io::WritePcap class as needed.
        //!
        //! Once loaded into Wireshark, packets from all interfaces will be
        //! interleaved. Use display filters to select a specific interface,
        //! e.g., `frame.interface_id==1` or `frame.interface_name=="eth0"`.
        class WritePcapInterface : public satcat5::io::WriteableRedirect {
        public:
            ...
            WritePcapInterface(
                ...
                const char* label, u16 type = LINKTYPE_ETHERNET);
