...
static inline u32 wide_write(Register& reg, s64 offset) {
    u64 tmp = (u64)offset;
...
void PtpReference::clock_rate(s64 offset) {
    ...
    (void)wide_write(m_reg, m_rate.convert(offset));
    m_offset = offset;
}

Time PtpReference::clock_now() {
    return satcat5::ptp::TIME_ZERO; // Not supported
}

void PtpReference::clock_rate_raw(s64 rate) {
    // Note: Cast to void prevents unused-value warnings.
    (void)wide_write(m_reg, rate);
    m_offset = m_rate.invert(rate);
}
...
void PtpRealtime::clock_rate(s64 offset) {
    ...
    (void)wide_write(tmp, m_rate.convert(offset));
    m_offset = offset;
}

void PtpRealtime::clock_rate_raw(s64 rate) {
    // Note: Cast to void prevents unused-value warnings.
    Register tmp = m_reg + RTC_RATE;
    (void)wide_write(tmp, rate);
    m_offset = m_rate.invert(rate);
}
