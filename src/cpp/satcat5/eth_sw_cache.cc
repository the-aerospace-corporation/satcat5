...
SATCAT5_PMASK_TYPE SwitchCacheInner::destination_mask(const PluginPacket& pkt) {
...
void SwitchCacheInner::query(PluginPacket& pkt) {
    ...
    if (pkt.dst_mask == 0) pkt.drop(SwitchLogMessage::DROP_NO_ROUTE);
}
