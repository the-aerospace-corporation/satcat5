...
//!
//! The `HdlcEncoder` accepts a Writeable interface, inserts inter-frame
//! "flag" tokens, appends a 16-bit or 32-bit checksum, and finally performs
//! byte-stuffing for reserved token values.
//!
//! The `HdlcDecoder` performs the inverse, accepting an HDLC stream one
//! byte at a time through the Writeable interface, decoding the result,
...
namespace satcat5 {
    namespace io {
        //! Inline HDLC encoder (framing layer only). \see codec_hdlc.h.
        ...
        //! Inline HDLC decoder (framing layer only). \see codec_hdlc.h.
