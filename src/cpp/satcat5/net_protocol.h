...
namespace satcat5 {
    namespace net {
        //! A net::Protocol is the counterpart to net::Dispatch that handles a
        //! particular data stream, identified by an associated net::Type.
        //!
        //! The Dispatch layer maintains a list of active Protocols, and
        //! inspects each incoming packet to route it to the appropriate
        //! destination. Protocols may be endpoints with application-layer
        //! functionality. They may also be middleware that does additional
        //! sorting. e.g., The udp::Dispatch class is both a Protocol (it
        //! accepts all UDP traffic from ip::Dispatch) and a Dispatch (it routes
        //! UDP packets to the appropriate port).
        //! \see net_dispatch.h
        class Protocol {
        public:
            //! Dispatch calls frame_rcvd(...) for each incoming frame with
            //! with a matching net::Type value.  The child class SHOULD read
