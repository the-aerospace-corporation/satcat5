...
namespace satcat5 {
    namespace net {
        //! AeroCube File Transfer Protocol (AeroFTP) transmitter.
        //!
        //! This file implements the transmit portion of a lightweight
        //! file-transfer protocol that can operate over unidirectional
        //! links. Transport is over raw-Ethernet or UDP.
        //!
        //! For a matching receiver, \see "hal_posix/file_aeroftp.h".
        //!
        //! Since data transmission is unidirectional, reliable file
        //! transfer requires an asynchronous side-channel that can request
        //! retransmission of missing blocks. This may be real-time, or it
        //! may occur hours or days later. If no such side-channel exists,
        //! sending the file multiple times may provide an acceptable chance
        //! of receiving the complete file.
        //!
        //! Incoming and outgoing files are divided into non-overlapping
        //! "blocks". Blocks are numbered from zero, starting on 1,024-byte
        //! boundaries. (i.e., All except the final block are exactly 1,024
        //! bytes long.) Files whose length is not a multiple of four bytes
        //! will be zero-padded. In theory the protocol supports files up to
        //! 16 GiB; this implementation has not been tested beyond 2 GiB.
        //!
        //! By default, the client sends one block every millisecond
        //! (~8.1 Mbps). To reduce this rate, call throttle(N) to wait N msec
        //! before sending each subsequent packet, yielding 8192000/N bps.
        ...
        protected:
            ...
            //! \see eth::AeroFtpClient, udp::AeroFtpClient.
            ...
            satcat5::io::Readable* m_src;   //!< Source for file data
            satcat5::io::Readable* m_aux;   //!< Source for retransmit control
    ...
    namespace eth {
        //! Recommended EtherType for AeroFTP over Ethernet.
    ...
    namespace udp {
        //! Recommended port number for AeroFTP over UDP.
        constexpr satcat5::udp::Port PORT_AEROFTP = {0x4346};

        //! Implement AeroFTP protocol over UDP.
        //! Note: Unidirectional UDP requires some care because of ARP
        //! prerequisites. In some cases, proxy-ARP or manual routing
