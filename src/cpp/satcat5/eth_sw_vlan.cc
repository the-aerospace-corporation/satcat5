...
bool SwitchVlanInner::query(PacketMeta& pkt)
{
    ...
    if (!tag_ok) {m_switch->free_packet(pkt.pkt); return false;}
    ...
    if (cost > policy.tcount) {
        ...
        if ((vpol == VPOL_DEMOTE) || (vpol == VPOL_AUTO)) pkt.pkt->m_priority = 0;
        if ((vpol == VPOL_STRICT) || (vpol == VPOL_AUTO && pkt_dei)) pkt.dst_mask = 0;
