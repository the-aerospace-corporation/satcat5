...
namespace satcat5 {
    namespace ip {
        ...
        //! For additional connection metadata, \see ip::Address.
        struct Addr {
        ...
        struct Header {
            ...
            //! Given a before/after change to any field in this IPv4 header,
            //! update the header checksum by applying the method of IETF
            //! RFC1624 Section 3, i.e., "HC' = ~(~HC + ~m + m')".
            //! Variants are provided for changing 16-bit and 32-bit fields.
            //!{
            ...
            //! This method reads the first 20 bytes of an IPv4 header, which
            //! contains basic header fields but not variable-length options
            //! (i.e., IHL > 5).  This is used in cases where the full header
            //! is unavailable or unnecessary (e.g., due to "peek" limits). To
            //! read options and validate the header checksum, \see read_from.
            ...
            //! header options and validates the IPv4 header checksum.
            //! \returns True if header and checksum are valid, false otherwise.
            bool read_from(satcat5::io::Readable* rd);  // Entire header (up to 64 bytes)
        };

        //! Calculate the IP header checksum over a block of data.
        //! For a valid header, the returned value should be equal to zero.
