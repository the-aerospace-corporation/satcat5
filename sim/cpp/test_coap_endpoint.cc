...
private:
    bool send_echo(Reader& msg, satcat5::io::Writeable* dst) {
        // Write the message to the test buffer.
        coap::Writer reply(last_sent.open());
        reply.write_header(
            (mode == Reply::ECHO_SEP) ? coap::TYPE_CON : coap::TYPE_ACK,
            coap::CODE_VALID, msg.msg_id(), msg.token(), msg.tkl());
        ...
        return ok && last_sent.send_to(dst);
    }

    bool send_reset(Reader& msg, satcat5::io::Writeable* dst) {
        // Write the message to the test buffer.
        coap::Writer reply(last_sent.open());
        reply.write_header(
...
typedef TestEndpoint<satcat5::coap::EndpointUdpHeap>    TestEndpointUdp;
...
TEST_CASE("coap_endpoint_udp") {
    ...
    SECTION("basic") {
        // Client and server setup.
        TestEndpointUdp client(Reply::NONE,     &xlink.net0.m_udp, 1);
        TestEndpointUdp server(Reply::ECHO_NOW, &xlink.net1.m_udp, 1);
    ...
    SECTION("concurrent") {
        ...
        // Client and server setup.
        TestEndpointUdp client(Reply::NONE,     &xlink.net0.m_udp, 3);
        TestEndpointUdp server(Reply::ECHO_NOW, &xlink.net1.m_udp, 2);
        server.bind(PORT_COAP);
        ...
        // Wait for ARP resolution, then send two confirmable requests.
        xlink.timer.sim_wait(1000);
        CHECK(client.send_request(c1, coap::TYPE_CON, coap::CODE_GET, 32));
    ...
    SECTION("multicast") {
        ...
        CHECK(client.connection(0)->is_request());
        CHECK(server.connection(0)->is_response());
    ...
    SECTION("reset") {
        // Client and server setup.
        TestEndpointUdp client(Reply::NONE,     &xlink.net0.m_udp, 1);
        TestEndpointUdp server(Reply::RESET,    &xlink.net1.m_udp, 1);
        server.bind(PORT_COAP);
        // Open connection + wait for ARP resolution.
        Connection *c1 = 0;
        REQUIRE((c1 = client.connect(xlink.IP1, PORT_COAP)));
    ...
    SECTION("separated") {
        // Client and server setup.
        TestEndpointUdp client(Reply::NONE,     &xlink.net0.m_udp, 1);
        TestEndpointUdp server(Reply::ECHO_SEP, &xlink.net1.m_udp, 1);
        server.bind(PORT_COAP);
        // Open connection + wait for ARP resolution.
        Connection *c1 = 0;
        REQUIRE((c1 = client.connect(xlink.IP1, PORT_COAP)));
...
TEST_CASE("coap_endpoint_multi") {
    ...
    TestEndpointUdp client_udp(Reply::ECHO_NOW, &xlink_udp.net0.m_udp, 1);
    TestEndpointUdp server_udp(Reply::ECHO_NOW, &xlink_udp.net1.m_udp, 1);
