...
// Known-good CCSDS-AOS transfer frame:
static const Packet REF_C1 = {
    0x66, 0x7F, 0x00, 0x09, 0x28, 0x00, 0xDC, 0xDC, 0xDC, 0xDC, 0xDC, 0xDC, 0xDC,
...
TEST_CASE("crc16-ccsds") {
    // Simulation infrastructure.
    SATCAT5_TEST_START;

    // All units under test write to the same buffer.
    satcat5::io::PacketBufferHeap buff;
    satcat5::crc16::XmodemRx uut_rx(&buff, 0xFFFF);
    satcat5::crc16::XmodemTx uut_tx(&buff, 0xFFFF);

    // Repeat every test for each of the example packets...
    auto pkt = GENERATE(REF_C1);
