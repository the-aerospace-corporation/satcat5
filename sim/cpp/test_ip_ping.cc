...
TEST_CASE("Ping") {
    ...
    SECTION("arp-simple") {
        ...
        xlink.timer.sim_wait(500);    // Wait for 1st ping + response
        CHECK(log.contains("Reply from")); log.clear();
        xlink.timer.sim_wait(1000);   // Wait for 2nd ping + response
        CHECK(log.contains("Reply from")); log.clear();
        xlink.timer.sim_wait(1000);   // Wait for 3rd ping + response
        CHECK(log.contains("Reply from")); log.clear();
        xlink.timer.sim_wait(1000);   // Wait for 4th ping + response
        CHECK(log.empty());
    }
    ...
    SECTION("arp-badip") {
        ...
        xlink.timer.sim_wait(1500);   // Bad IP, no ARP response
    ...
    SECTION("icmp-simple") {
        ...
        xlink.timer.sim_wait(500);    // Wait for ARP handshake...
        CHECK(log.empty());
        xlink.timer.sim_wait(1000);   // Wait for 1st ping + response
        CHECK(log.contains("Reply from")); log.clear();
        xlink.timer.sim_wait(1000);   // Wait for 2nd ping + response
        CHECK(log.contains("Reply from")); log.clear();
        xlink.timer.sim_wait(1000);   // Wait for 3rd ping + response
